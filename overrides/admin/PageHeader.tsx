...
const ManageAccountDropdownItem = () => {
  ...
  if (isFeatureDisabled(Feature.AccountV3)) return null;
  return <DropdownItem key="manage account" id="manage-account" onClick={() => keycloak.accountManagement()}>{t("manageAccount")}</DropdownItem>;
};

const ServerInfoDropdownItem = () => {
  const { realm } = useRealm();
  const { t } = useTranslation();
  return <DropdownItem key="server info" component={(props) => <Link {...props} to={toDashboard({ realm })} />}>{t("realmInfo")}</DropdownItem>;
};

const ClearCachesDropdownItem = () => {
  const { t } = useTranslation();
  const [open, toggleModal] = useToggle();
  return <><DropdownItem key="clear caches" onClick={() => toggleModal()}>{t("clearCachesTitle")}</DropdownItem>{open && <PageHeaderClearCachesModal onClose={() => toggleModal()} />}</>;
};

const HelpDropdownItem = () => {
  const { t } = useTranslation();
  const { enabled, toggleHelp } = useHelp();
  return <DropdownItem data-testId="helpIcon" icon={<HelpIcon />} onClick={toggleHelp}>{enabled ? t("helpEnabled") : t("helpDisabled")}</DropdownItem>;
};
...
const kebabDropdownItems = (isMasterRealm: boolean, isManager: boolean) => [
  ...
  ...(isMasterRealm && isManager ? [<ClearCachesDropdownItem key="Clear Caches" />] : []),
...
export const Header = () => {
  ...
  return (
    <KeycloakMasthead
      ...
      brand={{ href: logoUrl, src: customLogo?.trim() ? customLogo : brandMarkUrl, alt: brandName, className: "keycloak__pageheader_brand" }}
      dropdownItems={userDropdownItems(isMasterRealm, isManager)}
      kebabDropdownItems={kebabDropdownItems(isMasterRealm, isManager)}
      toolbarItems={[<ToolbarItem key="help" align={{ default: "alignRight" }} visibility={{ default: "hidden", md: "visible" }}><HelpHeader /></ToolbarItem>]}
    />
  );
};
