...
const ManageAccountDropdownItem = () => {
  ...
  return (
    <DropdownItem key="manage account" id="manage-account" onClick={() => keycloak.accountManagement()}>
      {t("manageAccount")}
    </DropdownItem>
  );
};
...
const ServerInfoDropdownItem = () => {
  ...
  return (
    <DropdownItem key="server info" component={(props) => <Link {...props} to={toDashboard({ realm })} />}>
      {t("realmInfo")}
    </DropdownItem>
  );
};
...
const ClearCachesDropdownItem = () => {
  ...
  return (
    <>
      <DropdownItem key="clear caches" onClick={() => toggleModal()}>{t("clearCachesTitle")}</DropdownItem>
...
export const Header = () => {
  ...
  return (
    <KeycloakMasthead
      ...
      brand={{
        href: logoUrl,
        src: customLogo?.trim() ? customLogo : brandMarkUrl,
        alt: brandName,
